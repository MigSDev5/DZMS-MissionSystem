...
private ["_missName","_coords","_wreck","_trash","_trash1","_trash2","_trash3","_trash4","_trash5","_veh1","_veh2","_vehicle","_vehicle1","_crate","_crate1","_marker","_markerA","_markerB","_group1","_group2","_group3","_group4","_endMission","_status"];
...
DZMSInProgress set [count DZMSInProgress,"EM1"];
...
_crate = createVehicle [if (DZMSEpoch) then {"USVehicleBox"} else {"AmmoBoxBig"},[(_coords select 0) - 1.5547,(_coords select 1) + 2.3486,0],[], 0, "CAN_COLLIDE"];
[_crate,"supply"] ExecVM DZMSBoxSetup;
[_crate] call DZMSProtectObj;

_crate1 = createVehicle [if (DZMSEpoch) then {"USLaunchersBox"} else {"AmmoBoxBig"},[(_coords select 0) + 0.3428,(_coords select 1) - 1.8985,0],[], 0, "CAN_COLLIDE"];
...
_endMission = {
    ...
    DZMSInProgress = DZMSInProgress - ["EM1"];
};

//DZMSAISpawn spawns AI to the mission.
//Usage: [_coords, count, skillLevel, unitArray]
_group1 = [[(_coords select 0) - 10.5005,(_coords select 1) - 2.6465,0],6,1] call DZMSAISpawn;
_group2 = [[(_coords select 0) + 4.7027,(_coords select 1) + 12.2138,0],6,1] call DZMSAISpawn;
_group3 = [[(_coords select 0) + 2.918,(_coords select 1) - 9.0342,0],4,1] call DZMSAISpawn;
_group4 = [[(_coords select 0) + 2.918,(_coords select 1) - 9.0342,0],4,1] call DZMSAISpawn;
